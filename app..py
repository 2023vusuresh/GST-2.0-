...
.main-header h1 {
    font-size: 2.8rem;
    font-weight: 800;
    background: linear-gradient(90deg, #f7971e, #ffd200, #21d190);
...
input[type="text"]:focus, .stTextInput input:focus {
    ...
    box-shadow: 0 0 0 3px rgba(255,210,0,0.15) !important;
}
...
.badge-exempt  { background: rgba(72,199,142,0.2); color: #48c78e; border: 1.5px solid #48c78e55; }
.badge-5       { background: rgba(33,209,144,0.15); color: #21d190; border: 1.5px solid #21d19055; }
.badge-12      { background: rgba(255,210,0,0.15); color: #ffd200; border: 1.5px solid #ffd20055; }
.badge-18      { background: rgba(247,151,30,0.15); color: #f7971e; border: 1.5px solid #f7971e55; }
.badge-28      { background: rgba(252,75,108,0.2); color: #fc4b6c; border: 1.5px solid #fc4b6c55; }
.badge-other   { background: rgba(160,174,192,0.15); color: #a0aec0; border: 1.5px solid #a0aec055; }
...
.not-found h3 { color: #a0aec0; margin: 0.5rem 0; }

/* Error */
.error-box {
    background: rgba(252,75,108,0.1);
    border: 1px solid rgba(252,75,108,0.3);
    border-radius: 12px;
    padding: 1rem 1.5rem;
    color: #fc4b6c;
    font-size: 0.9rem;
...
if code_input and code_input.strip():
    ...
    if len(matches) == 0:
        ...
        if len(matches) == 1:
            ...
            st.markdown(f'<p style="color:#a0aec0;font-size:0.9rem;">🔎 Found <b style="color:#ffd200">{len(matches)}</b> matching codes. Showing exact match or first result.</p>', unsafe_allow_html=True)
        ...
        <div class="result-card">
          ...
          <div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:1rem; margin-bottom:1.2rem;">
            <div>
              <div style="color:#718096;font-size:0.75rem;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;">
                {'HSN Code (Goods)' if code_type == 'HSN' else 'SAC Code (Services)'}
              </div>
              <div style="color:#fff;font-size:2rem;font-weight:800;letter-spacing:0.05em;line-height:1.1;">
                {row['code']}
              </div>
            </div>
          ...
          <!-- Description -->
          <div style="background:rgba(255,255,255,0.04);border-radius:12px;padding:1rem 1.2rem;margin-bottom:1.5rem;">
            <div style="color:#718096;font-size:0.72rem;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;margin-bottom:0.4rem;">Description</div>
            <div style="color:#e2e8f0;font-size:0.95rem;line-height:1.6;">{description}</div>
          </div>
          ...
          <div class="info-row">
            ...
            <span class="info-val" style="color:#ffd200;font-weight:700;">{igst_v if is_inter else f'CGST {cgst_v} + SGST {sgst_v}'}</span>
          </div>
        ...
        if len(matches) > 1:
            ...
            st.markdown(f'<div class="section-title">📋 All {len(matches)} matches for "{query}"</div>', unsafe_allow_html=True)
    ...
    <div style="text-align:center;padding:3rem 1rem;color:#4a5568;">
      ...
      <div style="color:#718096;font-size:1.05rem;font-weight:500;">Enter an HSN or SAC code above to get started</div>
...
<div style="text-align:center;color:#4a5568;font-size:0.8rem;padding:1rem 0 2rem;">
